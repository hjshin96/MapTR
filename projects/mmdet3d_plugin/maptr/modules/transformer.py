...
class BAEFormer(BaseModule):
    ...
    def __init__(self,
        ...
        if modality == 'fusion':
            self.fuser = build_fuser(fuser) #TODO
        self.use_attn_bev = encoder['type'] == 'BAEFormerEncoder'
        self.encoder = build_transformer_layer_sequence(encoder)
        self.decoder = build_transformer_layer_sequence(decoder)
        self.embed_dims = embed_dims
        self.num_feature_levels = num_feature_levels
    ...
    def attn_bev_encode(
            ...
            **kwargs):
        img = img_inputs["img"]
        img_backbone = img_inputs["img_backbone"]
        bs = img[0].shape[0]
        bev_queries = bev_queries.unsqueeze(1).repeat(1, bs, 1)
        bev_pos = bev_pos.flatten(2).permute(2, 0, 1)
        ...
        feat_flatten = []
        spatial_shapes = []
        for lvl, feat in enumerate(img_inputs['img_backbone']['out_indices']):
            bs, num_cam, c, h, w = feat.shape
            spatial_shape = (h, w)
            feat = feat.flatten(3).permute(1, 0, 3, 2)
            if self.use_cams_embeds:
