...
class MapTRDepth(MVXTwoStageDetector):
    """MapTR.
    Args:
    ...
    def __init__(
        ...
        super(MapTRDepth, self).__init__(
            pts_voxel_layer,
            pts_voxel_encoder,
            pts_middle_encoder,
            pts_fusion_layer,
    ...
    def forward_train(
        ...
        prev_img = img[:, :-1, ...]  # Temporal fusion 위한 변수인거 같은데 안쓰는듯
        ...
        prev_bev = self.obtain_history_bev(prev_img, prev_img_metas) if len_queue > 1 else None     # Temporal fusion 위한 변수인거 같은데 안쓰는듯
        
        img_metas = [each[len_queue - 1] for each in img_metas]
        img_feats = self.extract_feat(img=img, img_metas=img_metas) # [mlvl, bs, num_cam, D, H, W] mlvl은 list로 나뉘어짐
        ### chgd
        
        depth_feats, img_feats = self.depth_net(img_feats, img_metas)
        depth_losses = self.depth_net.get_depth_loss(gt_depth, depth_feats)
...
@DETECTORS.register_module()
class MapTRDepth_fp16(MapTRDepth):
    """
